...
#[derive(Debug, Clone)]
struct AgeGenderMap(pub HashMap<(Age, Gender), Count>);
#[derive(Debug, Clone, Default)]
struct CohortFertility(pub BTreeMap<Year, CohortData>);
...
impl AgeGenderMap {
	...
	#[allow(unused)]
	fn count_gender(&self, gender: Gender) -> Count {
		self.0
			.iter()
	...
	#[allow(unused)]
	fn count_age(&self, age: Age) -> Count {
		self.0
			.iter()
	...
	#[allow(unused)]
	fn count_age_gender(&self, age: Age, gender: Gender) -> Count {
		self.0[&(age, gender)]
	}
}
...
struct SimulationResult {
	initial_population: AgeGenderMap,
	final_population: AgeGenderMap,
...
impl PopulationSimulator {
	...
	fn handle_births(&mut self, year: Year) {
		...
		let newborns = self
			...
			.0
			.iter()
			.filter_map(|(&(age, gender), &females)| {
				(gender == Gender::Female).then(|| {
					(
						age,
